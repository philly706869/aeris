...
- **AERIS**(에리스)는 프로그래밍 언어와 그 관련 인프라를 함께 개발하는
  프로젝트다. 정식 언어 명칭은 **AERIS Standard**지만, 대부분의 문맥에서는
  AERIS라고 부른다. 문맥에 따라 AERIS는 언어 자체 또는 언어 구현에 필요한
  인프라 전체를 가리킬 수 있다.
- AERIS Standard는 다중 패러다임 범용 언어이며, 다른 주요 프로그래밍 언어를
  대체할 수 있는 수준을 장기 목표로 한다. 학습용 프로젝트가 아니라 실제 사용을
  목적으로 한다.
- 핵심 철학은 **"절대 외부 입력, 개발자를 신뢰하지 않는다"**이다. 컴파일은
  증명된 사실 또는 공리에만 기반해야 하며, 매우 엄격하면서도 유연한 타입 시스템과
  함수 호출 등의 전제 조건을 표현하고 검증하는 자체 증명/전제 요구 시스템을
  제공한다. 예를 들어 함수 호출에 `a > 0` 같은 조건을 요구할 수 있다. 가능한 것은
  최대한 컴파일타임에 처리하고 최적화하며, 런타임 처리는 차선책으로 둔다.
- 성능과 확장성도 핵심 가치다. 기본 배포 형태는 바이너리 컴파일이며, `libc`나
  Visual Studio Runtime 같은 런타임 의존성을 가능한 한 줄인다. 부트스트랩 이전에는
  Linux x86_64만 지원하고, 이후 여러 환경을 지원한다. JIT과 REPL 지원은 후속
  단계에서 추가한다.
- 구현은 LLVM 기반이며 Rust의 Inkwell을 사용한다. 현재는 부트스트랩 이전의
  프로토타입 단계로 대부분의 기능은 아직 구현되지 않았고, 1인 개발로 진행 중이다.
- 라이선스는 MIT다. 부트스트랩 이전에는 외부 기여를 대부분 받지 않는다.
- 언어 문법 전체를 정리한 정본 문서는 아직 없으며, 일부 정보를 담은 파일을 바탕으로
  이를 작성해야 한다. 현재 최우선 개발 대상은 `cluster!`를 포함한 파서/UI 프레임워크다.
- `unsafe`에 해당하는 탈출구와 인라인 어셈블리는 가능한 한 배제하며, 공식 인라인
  어셈블리 지원은 계획하지 않는다. 분기는 가급적 전제/증명으로 제거하고, 증명이
  어렵거나 신뢰할 수 없는 값에는 `if` 검사 또는 공리를 쓴다. FFI 입력은 외부 입력으로
  보고 반드시 런타임 검사한다.
- 메모리 관리는 GC 없이 전제/증명 기반으로 처리하며 Rust와 유사한 방향을 지향한다.
  참조 계수 같은 비용도 컴파일타임에 계산·제거할 수 있으면 최대한 최적화하고, 런타임
  확인이 불가피할 때만 실제 분기를 둔다.
- 컴파일러 확장 또는 유사한 메커니즘으로 문법을 추가할 수 있어야 한다. 자체 문법
  정의 프레임워크는 문법 정의와 함께 오류 복구 지점, 하이라이팅, 호버 이벤트,
  포맷 규칙 등 강력한 에디터 지원 설정을 제공하는 방향이다. 여기서 UI는 일반적인
  User Interface를 뜻하며, 파일 코드뿐 아니라 확장/플러그인으로 생성되는 코드까지
  포함해 실제 사용자와 상호작용하는 텍스트 코드 처리와 IDE 지원을 담당한다.
- 컴파일러 확장과 플러그인도 동일하게 증명과 검증을 거쳐야 한다. 공리의 확장은
  가능하되 최대한 지양한다. 일반 플러그인에는 공리 정의 권한을 주지 않고 특수
  플래그가 있는 경우만 허용하는 등의 권한 모델을 고려 중이지만, 세부 정책은 미확정이다.
